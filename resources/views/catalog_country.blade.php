...
<body>
    ...
    <div class="container mt-5 d-flex flex-wrap justify-content-center gap-1">
        ...
        <div class="container d-flex flex-column flex-wrap gap-2 ms-5" style="max-width: 70% !important">
            @forelse ($object as $objec)
                <div class="hotel-card border rounded-2 p-2 ">
                    <div class="row">
                        <div class="col-sm-3">
                            <img src="/storage/images/hotels/{{ $objec->photo }}" class="img-thumbnail rounded-start"
                                alt="">
                        </div>
                        <div class="col-sm">
                            <p class="card-title fs-3 fw-bold ">{{ $objec->title_object }}
                            <p class="location">{{ $objec->title_object }}</p>
                            </p>
                            <p class="truncate3 card-text">{{ $objec->description }}</p>
                        </div>
                        <div class="col-sm-4">
                            <p class="cost text-end me-4 fs-4">{{ $objec->apartament_object->cost }} руб</p>
                            <p class="text-end me-4"><a href="{{ route('hotelcard', ['id' => $objec->id]) }}"
